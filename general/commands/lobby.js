...
module.exports = async (msg, type) => {
  ...
  if (!msg.guild) {
    return msg.reply(
      "While a single player deception game would be really easy, it's best played in a server where there are, yunno, people you can play with."
    )
  }
  ...
  if (runningGames[msg.guild]) {
    return msg.reply(
      "Sorry, a discord deception game is already happening in this server. You'll have to wait for the current one to finish before starting a new one."
    )
  }
  ...
  game.lobbyMsg = sUmsg
  msg.reply(
    "You're the host. Use **?ready** when everyone has joined. If you change your mind, use **?cancel**"
  )
